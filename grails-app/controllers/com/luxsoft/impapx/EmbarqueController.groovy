...
class EmbarqueController {
	...
	def eliminarPartidas(){
		...
		JSONArray jsonArray=JSON.parse(params.partidas);
		jsonArray.each{
			def embarqueDet=EmbarqueDet.get(it.toLong())
	...
	def relacionDeContenedores(){
		...
		/*
		def reportDef = new JasperReportDef(name:'RelacionDeContenedoresPorEmbarque',
			fileFormat:JasperExportFormat.PDF_FORMAT,parameters:[ID:params.id] 
			)
		
		def out=jasperService.generateReport(reportDef)
		
		response.setHeader("Content-disposition", "attachment; filename=\"RelacionDeContenedores." + reportDef.fileFormat.extension + "\"");
